...
async function seedDatabase() {
  try {
    ...
    for (let gen = 1; gen <= 8; gen++) {
      ...
      const genData: GenerationAPIResponse = await fetchFromPokeAPI(`generation/${gen}`);
      
      // Sort Pokemon species by ID
      const sortedSpecies = [...genData.pokemon_species].sort((a, b) => 
        getIdFromUrl(a.url) - getIdFromUrl(b.url)
      );
      ...
      for (const species of sortedSpecies) {
        ...
        if (existing) {
          console.log(`Pokemon #${pokemonId} (${species.name}) already exists, skipping...`);
          continue;
        }
        
        console.log(`Fetching data for Pokemon #${pokemonId} (${species.name})...`);
        
        try {
          const pokemonData: PokemonAPIResponse = await fetchFromPokeAPI(`pokemon/${pokemonId}`);
          ...
          await db.insert(schema.pokemon).values({
            ...
            speciesId: pokemonId // Use the same ID since species is redundant
          });
          
          // 2. Insert Pokemon-Generation relationship
          await db.insert(schema.pokemonGenerations).values({
            pokemonId: pokemonId,
            generationId: gen
          }).onConflictDoNothing();
          ...
          console.log(`Successfully seeded Pokemon #${pokemonId} (${pokemonData.name})`);
        } catch (err) {
