  ...
  try {
    ...
    const result: Pokemon = {
      ...
      species: {
        ...
        url: `http://localhost:3000/species/${pokemonData.speciesId}`
      }
    };
    ...
    const pokemonList = await Promise.all(
      pokemonIds.map(async (id) => {
        ...
        return {
          name: pokemonData.name,
          id: pokemonData.id,
          image: (pokemonData.sprites as any).front_default || `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${pokemonData.id}.png`
        };
      })
    );
